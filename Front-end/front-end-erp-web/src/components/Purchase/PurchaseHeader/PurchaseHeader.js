...
import { deleteProvider } from "../../../services/purchaseServices"
...
function PurchaseHeader(props) {
    ...
    const handleDeleteQuotes = async (listQuote) => {
        if (listQuote && listQuote.length > 0) {
            let res = {}
            if (url === '/manage-purchase/providers') {
                res = await deleteProvider(listQuote)
            } else {
    ...
    const handleCreateNewItem = () => {
        history.push('/manage-purchase/new')
    }

    const handleCreateNewCustomer = () => {
        history.push('/manage-purchase/providers/create')
    }
    ...
    return (
        <>
            <div className='wrapper-sales-header'>
                    ...
                    <div className='wrap-content-left'>
                        {url === '/manage-purchase/providers'
                            ...
                            <>
                                ...
                                <span className='title-create-item'>Nhà cung cấp</span>
                            </>
                            :
                            <>
                                {url === '/manage-purchase/orders'
                                    ...
                                    <>
                                        ...
                                        <span className='title-create-item'>Đơn mua hàng</span>
                                    </>
                                    :
                                    <>
                                        <button onClick={() => handleCreateNewItem()} className='btn btn-purple btn-create-quote'><FormattedMessage id="sales-btn-new" /></button>
                                        <span className='title-create-item'>Yêu cầu báo giá</span>
                                    </>
                                }
                            </>
                        }
                ...
                {url === '/manage-purchase/providers' && props?.customers && props?.customers.length > 0 &&
