...
function ListCustomer() {
    ...
    const fetchAllCustomer = async (page, pageSize) => {
        const res = await getCustomerPagination(page, pageSize)
        if (res?.EC === 0) {
            setCustomer(res?.DT)
    ...
    const handleCheckAll = () => {
        ...
        if (currentStatus) {
            ...
            setSelectedItems(customers.map(item => item?.id?.toString()))
        }
    }
    ...
    const handleViewDetailCustomer = (customerId) => {
        history.push(`/sale-order/customers/${customerId}`)
    }

    return (

        <>
            <SalesHeader changeFilter={setSearchCustomer} selectedItems={selectedItems} handleUncheckAll={handleUnCheckAll}
                                                ...
                                                (() => {
                                                    ...
                                                    if (filteredCustomers.length > 0) {
                                                        return filteredCustomers.map((item, index) => (
                                                            <div onClick={() => handleViewDetailCustomer(item?.id)} key={index} className='customer-item d-flex col-4  '>
                                                                <div className='avatar-customer'>
                                                                    <img className='img-customer' src={avatarUser} alt='avatar-customer' />
                                                                </div>
                                                                <div className='des-customer'>
                                                                    <span className='name-customer '>{item?.fullName}</span>
                                                        ...
                                                        if (filteredCustomers.length > 0) {
                                                            return filteredCustomers.map((item, index) => (
                                                                <tr className='hover-item' key={uuidv4()} >
                                                                    ...
                                                                    <td onClick={() => handleViewDetailCustomer(item?.id)}>{item?.phone}</td>
                                                                    <td onClick={() => handleViewDetailCustomer(item?.id)}>{item?.email}</td>
                                                                    <td onClick={() => handleViewDetailCustomer(item?.id)}>{item?.address}</td>
                                                                </tr>
